...
public class JsonStubMappingCreator {
	...
	public void addMappingFrom(String mappingSpecJson) {
		if (mappingSpecJson.contains("mappings")) {
			Mappings buildFroms = Mappings.buildFrom(mappingSpecJson);
			List<StubMapping> mappings = buildFroms.getMappings();
			for (StubMapping stubMapping : mappings) {
				stubMapping.setTransient(false);
